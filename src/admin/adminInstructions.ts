...
const parseError = (error: any): string => {
  ...
  //this from the solana default or in built  errors
  const [index, errorCode] = error.InstructionError;
  
  /**
   * to get  error message we use an index of zero 
   * because with zero we can get the first element which is 
   * the error message .
   * then with we can pass it to the messages properties
   * 
   */
